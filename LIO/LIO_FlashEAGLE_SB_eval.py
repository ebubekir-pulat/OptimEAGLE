print("\n\n*******************************\nStarting LIO_FlashEAGLE_SB_eval.py\n\n")
...
sb_prompt_categories = [jsonObj.at[i, 'category'] for i in range(len(jsonObj))]
...
for model_index in models_to_test:
    ...
    for test_run in range(test_runs):
        ...
        for i in range(len(sb_prompts)):
            ...
            prompt_category = sb_prompt_categories[i]
            temp, top_p, top_k_gen = LIO_settings[prompt_category]
            ...
            for question in sb_prompts[i]:
                ...
                # Below 2 Code Lines From: https://github.com/SafeAILab/EAGLE
                output_ids = model.eagenerate(input_ids, temperature=temp, max_new_tokens=max_new_tokens,
                                              top_p=top_p, top_k=top_k_gen, log=True)
...
print("\n\n*******************************\nFinished Running LIO_FlashEAGLE_SB_eval.py\n\n")